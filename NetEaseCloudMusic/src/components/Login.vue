...
export default {
    ...
    methods:{
        async sendcode(){
        if(this.$refs.telenumber.value=='')
        {
            alert('请输入电话')
        }

       else{    const{data:res}=await this.$ajax.get('/captcha/sent',                  {
                params:{
                    phone:this.$refs.telenumber.value
                }
            })
             console.log(res)}
        ...
        async login(){
            ...
            if(this.$refs.telenumber.value=='' || this.$refs.code.value=='')
            {
                 alert('请输入电话/验证码')
            }
            
            else{ 
       const{data:res}=await this.$ajax.get('/login/cellphone?phone='+this.$refs.telenumber.value+'&captcha='+this.$refs.code.value       /*         {
                params:{
                    phone:this.$refs.telenumber.value,
                    password:123456,
